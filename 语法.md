...
同时，这也是泛指的一种用法，可以参阅<a href="#R2">这个</a>了解更多详情       
...
- 加了the
  ...
  - 表示该星期的确定要联系上下文确定
    ...
    这个句子的Tuesday加了the，其确认不能靠最近的星期，而是要联系上下文
  
    句子后半句提到after the accident，于是 on the Tuesday 就是那个事故发生那一周的周三了
...
当前面用at,by,after,before修饰时，往往不加冠词，比如说at noon

如果用在其他介词之后，或是在其他场合，需要加冠词

比如说

watch the dawn

如果是表示某一天的某个时间段，通常用on，并加不定冠词，还有修饰语进行修饰
...
如果三餐前有形容词，通常还要加不定冠词，来表示具体的一餐

We ofter have a big lunch and a nice dinner
...
接在by后面的交通名词，不需要加冠词，其他场合需要

| by+交通名词 | 其他场合         |
| ----------- | ---------------- |
| by bicycle  | take the bicycle |
| by bus      | be on the bus    |
| by car      | be on the car    |

特别的，on foot 前面不需要加冠词the,其他一般情况下，in、on、都需要用冠词限定



##### 关于通讯工具

by+通讯工具不需要加冠词，其他场合需要

| by+通讯工具  | 其他场合                 |
| ------------ | ------------------------ |
| by telephone | John is on the telephone |
| by radio     | a talk on the radio      |
| by post      | put a letter in the post |



##### 平行结构、

如果两个名词一起放在同一个平行结构里，即使是单数可数名词，也不加冠词
...
这些短语通常具有副词功能，用作状语，用来修饰动词，所以不需要加the
...
#### few/a few 和  a little/little

只能接复数名词：few、a few

直接接不可数名词： a little、little

表示肯定的：a few，a little

表示否定的：few、little

only搭配：a little,a few

不可和only搭配：little、few

very搭配：few,little 为了表示强调

#### some和any

相同点：可数复数和不可数名词连用，表示”一些“

不同点：some用在肯定句，any用在否定句、疑问句

> some也可以用在疑问句中，当你期待对方的回答是肯定的话
>
> Have you got some medicine to cure your cough?
>
> 你吃治咳嗽的药了吗？

##### any和可数单数搭配

这时候表示“无论哪一个，任何一个”，可以用于任何类型的句子，比如说肯定句
...
这里用了some搭配restaurant 表示某一家餐馆，很明显，一个人不能同时在几家餐馆吃饭

所以不是some restaurants

某些语境下，some+可数单数名词也可以表示强烈的感情色彩，往往暗示缺乏兴趣，甚至是蔑视的态度

示例：

- I don't want to spend my life in some lonely little village

  > 我可不想把我的一生浪费在一个偏僻的小山村
...
通常是some加数词再加名词，此时，some的意思是”大约“，这种用法通常暗示这是一个给人以深刻印象的很大的数字
...
这里的some，就是为了强调这些数字是多么可观

### 7.3.2 个体限定词

#### each和every

相同：each和every后面只能跟可数单数名词

如果修饰的名词做主语，很明显谓语动词要用单数

Every body has a gift

Each body has a gift

不同：

1. each不仅用于限定词，还可以作为代词，every只能用作限定词

    所以，each可以单独使用，但是every必须搭配名词
    ...
    > 每个男孩都有礼物
    >
    > each 作为代词，可以跟在主语后面，变成主语的同位语，但是主语boys是复数，谓语动词应该要用复数形式
    ...
    > each 作为代词也可以放在间接宾语的后面
    
2. each指两个及两个以上的事物，every只能指三个或三个以上的事物
...
3. every强调整体的意思，相当于all，而each则表示个别的概念

    We want every student to succeed in the exam

    > every强调整体，即希望全体学生都通过考试
    >
    > 我们希望所有的学生都能通过考试

    Each student has his own personal dream in his mind

    > each强调个体差别，每个人的梦想都是不一样的
    >
    > 每个学生心中都有自己的个人梦想

    **特别的：**each and every这张用法是既强调整体，又强调个体差别
...
4. 与数词的搭配关系
   ...
   each不与数词连用，every可与数词连用，然后后面再加复数名词

   Every two days

   > 每两天=每隔一天=Every other day

   也可以用every和序数词以及单数名词连用
...
- another不能修饰不可数名词及复数名词，只能和单数可数名词连用，表示不确定的”另一个“，”再一个“
...
- other可以和单数名词，复数名词，不可数名词连用，表示不确定的“另外的”，“其余的”人或物
...
- another:数词要放置在another后面

  结构：another+数词/few+复数名词
  
  意义：在原有基础上的一个延续，强调的是数量概念，是一个量的累计
  ...
  **特别的**：another不能和a连用，因此和a few 搭配时，只能说another few
  
  another few weeks
  
  > 还有几个星期
  
- other：数词应该放在other的前面
  ...
  I am always busy from Monday to Tuesday,But I can come on three other days
  
  > 我周一到周四总是很忙，但是我其他三天能来
  
  other 强调的是类别概念，指特定的某三天，不是一个数量的概念
  ...
  **other还可以和其他限定词搭配**
  ...
  | ----------------- | ------------------------------------------------------------ |
  | some              |                                                              |
  | any               | Have you get any other color?<br />你有其他的颜色吗？        |
  | every             | every other day<br />每隔一天                                |
  | enough            | you can have everything in life you want if you'll just help enough other people to get waht they want!<br />帮助别人梦想成真，你自己也就可以梦想成真 |
  | one/two/three/... | three other book<br />其他三本书，不要翻译成other three book |
  | no                | no other tools <br />没有其他的工具了                        |
  | many              | I have many other book for you to read<br />我还有许多颜色的书给你看<br />（其他许多书=other many book是错误的） |
  | his               |                                                              |
  | my                |                                                              |
  | their             |                                                              |
  ...
  **other和the other**
  ...
  other+名称表示不确定的其他的，另外的
  
  - some metals are mangnetic and other metals are not
  
    > 一些金属有磁性，而另一些金属没有
  
  the other 表示特定范围内剩下的其余的
  
  - Of the metals,only mercury is liquid and other metals are solid
  
    > 在金属中，只有水银是液体，其他金属都是固体
  
  > others和the others也分别适用
  >
  > - Of all metals,only mercury is liquid and the others are solid
  > - some metals are mangnetic and others are not
...
- 作为代词
  ...
  Either/Neither of+**限定词**+名词复数+单数谓语动词+....
...
- 意思
  ...
  - neither只有一个意思，两者都不
    ...
    **either....not相当于nether**
    ...
    Either of the answers is not correct=Neither of the answer is correct

#### 限定词的二和三

英语中有双数概念，

1. both和either,neither都只能用来指`二`
...
3. neither表示两者都不
   ...
   Neither of the twics was arrested,beacuse I saw both at a party last night

   > 这两个双胞胎都没有被抓，因为我昨晚在聚会上看到他们了
...
| 表示持续一段时间（for+时间段） | I have already lived here for third years                    |        |
